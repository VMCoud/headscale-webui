        ...
        if advertised_routes:
                    ...
                    if str(route_info["prefix"]) == str(route["prefix"]) and (
                        ...
                        if route_info["id"] != route["id"]:
                            ...
                            if route["enabled"] and route_info["enabled"]:
                                ...
                                app.logger.debug("Both routes are enabled.  Setting as HA [%s] (%s) ", str(machine["name"]),
                                                 str(route["prefix"]))
                                ha_enabled = True
            ...
            for route in pulled_routes["routes"]:
                ...
                if route["prefix"] != "0.0.0.0/0" and route["prefix"] != "::/0" and route["prefix"] not in failover_pair_prefixes:
                    app.logger.debug(
                        "Route:  [" + str(route['machine']['name']) + "] id: " + str(route['id']) + " / prefix: " + str(
                            route['prefix']) + " enabled?:  " + str(route['enabled']))
                    route_enabled = "red"
                    route_tooltip = 'enable'
                    if route["enabled"]:
...
def render_machines_cards():
    ...
    if LOG_LEVEL == "DEBUG":
        # DEBUG:  Do in a for loop:
        for idx in iterable:
            thread_machine_content(machines_list["machines"][idx], machine_content, idx, all_routes, failover_pair_prefixes)
    else:
